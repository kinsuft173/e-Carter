...
-(void)customUI:(NSMutableArray*)array
{
    ...
    UITapGestureRecognizer* tagG = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(goTapAction:)];
    
    [self.myScroll addGestureRecognizer:tagG];
...
- (void)goTapAction:(id)ges
{
    if (self.myScroll.contentOffset.x/SCREEN_WIDTH == (self.scrollCount - 1) ) {
        
        [self.delegate didSelectAdvertiseAtRow:0];
        
    }else{
        
        [self.delegate didSelectAdvertiseAtRow:self.myScroll.contentOffset.x/SCREEN_WIDTH];
